...
// JS signature: Equator(body, date, observer|null, ofdate:boolean, aberration:boolean)
function eclipticLongitude(body, date){
  const equ = Astro.Equator(body, date, null, true, true);
...
export function lahiriAyanamsa(date = new Date()){
  ...
  const base = 24.102;
  const driftPerYear = 50.290966/3600;
