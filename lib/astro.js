...
// Geocentric ecliptic longitude using Equator -> Ecliptic.
// IMPORTANT: pass Astro.Body[bodyName] (enum), not a raw string.
function eclipticLongitude(bodyName, date){
  const obs = new Astro.Observer(0, 0, 0); // neutral observer; swap for user lat/lon later
  const body = Astro.Body[bodyName];       // <-- enum value (fixes NaN)
